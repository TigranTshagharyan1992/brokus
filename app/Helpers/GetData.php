...
use App\AdminModels\EntityOption;
...
class GetData
{
    ...
    public static function getElement($id, $with=false)
    {
        $records = Entity::query();
        $records->where('entity_visible', 1);
        if(!is_numeric($id)) {
            $records->whereHas('entitySEO', function ($q) use ($id){
                $q->where('es_url', $id);
            });
        }else{
            $records->where('entity_id', $id);
        }
        if ($with) {
            $records->with($with);
        } else {
    ...
    public static function getData($filters = [], $sortings = [], $with = false, $limit = false)
    {
        $records = Entity::query();
        $records->where('entity_visible', 1);
    ...
    {
        $records = \App\AdminModels\Entity::query();
        $records->where('entity_visible = ', 1);

        foreach($filters as $field=>$value)
        {
            if (is_array($value)) {
                if ($field == self::OR_WHERE) {
                    foreach ($value as $relation => $data) {
                        if (is_array($data)) {
                            $records->orWhereHas($relation, function($q) use ($data){
                                foreach ($data as $key => $data) {
                                    if (is_array($data)) {
                                        if ($data[0] == self::OR_WHERE) {
                                            if (isset($data[2])) {
